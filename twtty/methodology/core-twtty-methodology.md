...
| **Spec** | A set of requirements, constraints, and acceptance criteria for the project or active Release Scope. The Spec MUST be precise, unambiguous, and deterministic enough that the same Spec produces the same approved Plan and EXECUTE outcomes on repeat application by the same or different competent agents (idempotence; see [Section 11.7](#117-rules)). The canonical baseline spec is saved in `<project-folder>/spec/spec.md`. Each subsequent Release Scope MUST have its own release-scoped spec at `<project-folder>/spec/spec-<release-id>.md` (see [Section 11.3](#113-on-new-release-scope)). |
...
| **Spec** | The Spec (requirements, constraints, and acceptance criteria) is approved AND meets the precision/determinism bar in [Section 11.7 — Specs MUST be precise and deterministic](#117-rules). |
...
- **Specs MUST be precise and deterministic.** Each Spec MUST be specified with sufficient precision that the same Spec yields the same approved Plan and EXECUTE outcomes on repeat application by the same or different competent agents (idempotence). To satisfy this rule, every requirement MUST be unambiguous, every constraint MUST be measurable or testable, and every acceptance criterion MUST be objectively verifiable (pass/fail without subjective judgment). Vague language (for example, "fast", "user-friendly", "robust") MUST be replaced with quantified or operationalized terms before SPEC stage exit. Where genuine non-determinism exists in the target system (for example, model outputs), the Spec MUST state the acceptable variation bounds.
